            ...
            <v-layout style='rgba(0,0,0,.2); border-radius: 15px' v-for='(item,index) in form.products_headings' my-5 pa-5>
                ...
                <template v-for='i,ii in item.pItems' >
                    ...
                    <v-autocomplete 
                        ...
                        :loading='!extras.products.loading && !extras.products[index][ii].loading ? !extras.products[index][ii].loading : false '
                        :items='!!extras.products[index][ii].loading && !!extras.products[index][ii].error ? !extras.products[index][ii].data : []'
                        v-validate="'required'"
...
,
    async mounted(){
        ...
        axios.get('clients',{params: {}})
         .then( (r) => {
            this.extras['products_headings'].data = this.generateItems(r.data.results,'product_id')
            this.extras['products_headings'].loading = false
            this.extras['products_headings'].error = false 
         } )
         .catch( (e) => {
            this.extras['products_headings'].error = true
            this.extras['products_headings'].loading = false
            this.extras['products_headings'].data = []
         })
