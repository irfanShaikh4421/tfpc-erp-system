...
Router.get('/', Handler.Bless(async (req,res,next) => {
   ...
   let sql = `SELECT 
                godown.godown_id, godown.name, godown.address , project.name as project_name
                FROM ${route.table} godown
                INNER JOIN projects project ON godown.project_id = project.project_id
